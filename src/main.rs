//! SimpleNote Desktop
//! 軽量・ポータブル・環境非依存なローカルメモ帳（egui/eframe・単一exe）。
//! SimpleTask Desktop とデータ構造・設計を共通化し、将来の統合を見据える。
...
fn already_running() -> bool {
    ...
    unsafe {
        let name = to_wide("SimpleNoteDesktop_Mutex");
        let handle = CreateMutexW(std::ptr::null(), 0, name.as_ptr());
        if handle.is_null() {
            return false;
        }
        if GetLastError() == ERROR_ALREADY_EXISTS {
            let title = to_wide("SimpleNote Desktop");
...
fn main() -> eframe::Result<()> {
    ...
    let mut viewport = egui::ViewportBuilder::default()
        .with_inner_size(saved.window_size.unwrap_or([640.0, 480.0]))
        .with_min_inner_size([280.0, 200.0])
        .with_title("SimpleNote Desktop");
